...
def add_emp(request):
    if request.method=='POST':
        ...
        new_emp=Empolyee(first_name=first_name,last_name=last_name,Salary=salary,bonus=bonus,phone=phone,dept_id=dept,role_id=role,hire_date=datetime.now())
        new_emp.save()
        ...
        return HttpResponse("An Exception occured Empolyee has not been added")
...
def filter_emp(request):
    if request.method=='POST':
        ...
        if dept:
            emps.filter(dept__name=dept)
        if role:
            emps.filter(role__name=role)
        context={
            'emps':emps
        }
        return render(request,'view_all_emp.html',context)
